...
routesCliente.post('/clientes', async (req, res) => {
  const {
    cgc_pessoa
  } = req.body;
  const prismaClientesRepositories = new PrismaClientesRepositories()
  const createClienteUseCase = new CreateClienteUseCase(prismaClientesRepositories)
...
routesCliente.put('/clientes', async (req, res) => {
  const { cgc_pessoa,
    ...
    conta,
    fornecedor,
    nu_fone_fornecedor  } = req.body;
  const prismaClientesRepositories = new PrismaClientesRepositories()
  const clienteUpdateData = new ClienteUpdateData(prismaClientesRepositories)

 
  clienteUpdateData.execute({
    cgc_pessoa,
    nm_pessoa,
    nm_fantasia_pessoa,
    rg_pessoa,
